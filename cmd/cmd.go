...
func NewKubeStraceCommand() *cobra.Command {
	...
	a := KubeStraceCommandArgs{}
	a.socketPath = flags.String("socket-path", "/run/crio/crio.sock", "The location of the CRI socket on the host machine. The defaults for common runtimes are as below: [ default /run/crio/crio.sock ] used to mount through")
	a.traceImage = flags.String("image", "quay.io/mwasher/crictl:0.0.2", "The trace image for use when performing the strace.")
